...
from open3d_example import join, get_file_list, make_clean_folder, draw_registration_result, sorted_alphanum
...
def compare_ending(str):
    result = False if str.endswith('.json') else True

def remove_json_files(ply_file_names):
    return list(filter(compare_ending, ply_file_names))
...
def run(config):
    ...
    ply_file_names = remove_json_files(ply_file_names)
    ply_file_names = sorted_alphanum(ply_file_names)
