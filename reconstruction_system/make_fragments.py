...
# examples/python/reconstruction_system/process_single_rgbd_image.py
...
def get_color_files(config):
    return [config['color_images']+'\\'+file for file in os.listdir(config['color_images'])]

def get_depth_files(config):
    return [config['depth_images']+'\\'+file for file in os.listdir(config['depth_images'])]
...
def run(config):
    ...
    color_files = get_color_files(config)
    depth_files = get_depth_files(config)
